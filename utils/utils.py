...
def plot_one_box(x, img, color=None, label=None, line_thickness=None,times=None):
    # Plots one bounding box on image img
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    print("**************",c1,c2)
    #cv2.rectangle(img,c1,c2,(0,0,0),2,cv2.LINE_AA)
    ...
    if label:
       ...
       # cv2.putText(img, times, (10,10), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
