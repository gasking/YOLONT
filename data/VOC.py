    ...
    def __call__(self, target, width, height):
        ...
        for obj in target.iter('object'):
            ...
            for i, pt in enumerate(pts):
                #这个会有一个bug  因为如果存在小数点的话就会报错
                cur_pt = int((bbox.find(pt).text).split('.')[0]) - 1
...
class VOCDetection(data.Dataset):
    ...
    def __init__(self, root,transform=[416,416], target_transform=VOCAnnotationTransform(),dataset_name='VOC'):
        self.root = root
        self.transform = transform
        self.target_transform = target_transform
        self.name = dataset_name
        #rootpath = r'D:\VOCdevkit\VOC2007'
        rootpath=VOC_ROOT
    ...
    def pull_item(self, index,mode=''):
        ...
        if mode=="":
         return torch.from_numpy(img).permute(2, 0, 1), target, height, width, offset, scale
            ...
            return torch.from_numpy(img).permute(2, 0, 1), target, height, width, offset, scale,img_id
        else:
            raise  ImportError("导出出错")
