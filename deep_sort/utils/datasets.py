...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        #cv2.imwrite("2.jpg",img0)
        return path, img, img0, self.cap#路径  处理好的图    未处理好的图  返回cap
...
class Load(object):
    #继承任一类
    def __inin___(self,path,img_size=None):
        self.video(path=path)
        self.tf,self.frame=self.cap.read()
        img = letterbox(self.frame, new_shape=320)[0]
        # Convert
        img = img[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
        img = np.ascontiguousarray(img)
        return  img,self.frame,self.cap#
    def video(self,path):
        self.cap=cv2.VideoCapture(path)
        self.count=self.cap.get(cv2.CAP_PROP_FRAME_COUNT)#这个是看这个视频有多少帧
        self.fps=self.cap.get(cv2.CAP_PROP_FPS)
        self.forcc=self.cap.get(cv2.CAP_PROP_FOURCC)
        self.size=self.cap.get(cv2.CAP_PROP_FRAME_WIDTH),self.cap.get(cv2.CAP_PROP_FRAME_HEIGHT)
        self.new_video=cv2.VideoWriter("output/"+path,self.forcc,self.fps,self.size)
#多管道传递
